...
export default function SakinahKitchenApp() {
  ...
  const handleLogin = async () => {
    if (!auth) return alert("Firebase belum diinisialisasi");
  ...
  const handleSaveProduct = async () => {
    if (!db) return alert("Firebase belum diinisialisasi");
    ...
    const payload = {
      ...
      image:
        productForm.image || "https://via.placeholder.com/150?text=No+Image",
  ...
  const handleCheckoutToWA = () => {
    ...
    message += `%0A*Total: ${formatRupiah(
      totalCartPrice
    )}*%0A--------------------------------%0A*Data Pengiriman:*%0A`;
    message += `👤 Nama: ${formData.name}%0A📱 No HP: ${
      formData.phone
    }%0A📍 Alamat: ${formData.address}%0A💳 Pembayaran: ${
    ...
    try {
            ...
            contents: [
              {
                parts: [
                  {
                    text: `Role: Asisten 'Sakinah Kitchen' yang gaul. User mood: "${aiMoodInput}". Pilih 1 menu dari: [${menuList}]. Jawab lucu & singkat.`,
                  },
                ],
              },
            ],
      ...
      setAiResponse(
        data.candidates?.[0]?.content?.parts?.[0]?.text ||
          "AI lagi ngemil, coba lagi ya!"
      );
    } catch (error) {
      setAiResponse("Koneksi error nih bestie!");
    } finally {
